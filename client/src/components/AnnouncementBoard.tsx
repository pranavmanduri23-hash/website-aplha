...
import { Plus, Trash2, Edit2, Check, X, Megaphone, Pin, Loader2 } from 'lucide-react';
...
export default function AnnouncementBoard({ isAdmin }: AnnouncementBoardProps) {
  const { rows, loading, insert, update, remove } =
    useSupabaseTable<Announcement>('announcements', { orderBy: 'created_at', ascending: false });
  ...
  const openEdit = (a: Announcement) => {
    setEditing(a); setTitle(a.title); setContent(a.content); setPinned(a.pinned);
    setShowDialog(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing) {
        await update(editing.id, { title, content, pinned });
        toast.success('Announcement updated');
      } else {
        await insert({ title, content, pinned, created_at: new Date().toISOString() } as Omit<Announcement, 'id'>);
  ...
  const handleDelete = async (id: number) => {
    try { await remove(id); toast.success('Announcement deleted'); }
    catch (e: unknown) { toast.error((e as Error).message ?? 'Delete failed'); }
  };

  const togglePin = async (a: Announcement) => {
    try { await update(a.id, { pinned: !a.pinned }); }
    catch (e: unknown) { toast.error((e as Error).message ?? 'Update failed'); }
  };
    ...
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          ...
          {sorted.map(a => (
            <div key={a.id} className="p-4 rounded-lg transition-all duration-300"
              style={{
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent style={{
          background: 'rgba(24,28,50,0.95)', backdropFilter: 'blur(12px)',
          ...
          <div className="space-y-4">
            ...
            <label className="flex items-center gap-3 cursor-pointer select-none">
              <div className={`w-10 h-5 rounded-full transition-colors duration-200 relative ${pinned ? 'bg-primary' : 'bg-muted'}`}
                onClick={() => setPinned(v => !v)}>
                <div className={`absolute top-0.5 w-4 h-4 rounded-full bg-white transition-transform duration-200 ${pinned ? 'translate-x-5' : 'translate-x-0.5'}`} />
              </div>
              <span className="text-sm text-foreground">Pin to top</span>
            </label>
            <div className="flex gap-2 pt-1">
              <Button variant="outline" onClick={() => setShowDialog(false)} className="flex-1" disabled={saving}>Cancel</Button>
